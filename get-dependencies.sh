...
if [ "${DEVEL_RELEASE-}" = 1 ]; then
    ...
    mkdir -p ./AppDir/bin
    cd ./CorsixTH
    cmake -DCMAKE_BUILD_TYPE=Release -DLUA_PROGRAM_PATH=/usr/bin/lua5.4 -DLUA_INCLUDE_DIR=/usr/include/lua5.4 -DLUA_LIBRARY=/usr/lib/liblua5.4.so -CMAKE_SOURCE_DIR=./CorsixTH.lua .
    make -j$(nproc)
